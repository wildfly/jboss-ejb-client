...
public final class DiscoveryEJBClientInterceptor implements EJBClientInterceptor {
    ...
    private static final Map<URI, Long> allowlist = new ConcurrentHashMap<>();
    private static final long ALLOWLIST_TIMEOUT = TimeUnit.MILLISECONDS.toNanos(SecurityUtils.getLong(SystemProperties.DISCOVERY_ALLOWLIST_TIMEOUT, 5000L));
    ...
    private void processMissingTarget(final AbstractInvocationContext context, final Exception cause) {
        ...
        if (shouldAllowlist(cause)) {
            addAllowListedDestination(destination);
        } else {
            addInvocationAllowListedDestination(context, destination);
        }
    ...
    static boolean shouldAllowlist(Exception cause){
    ...
    static void addInvocationAllowListedDestination(AbstractInvocationContext context, URI destination) {
        ...
        if (destination != null) {
            ...
            set.add(destination);
            if (Logs.INVOCATION.isDebugEnabled()) {
                Logs.INVOCATION.debugf("DiscoveryEJBClientInterceptor: Allowlisting destination for this invocation (locator = %s, weak affinity = %s, missing target = %s)", context.getLocator(), context.getWeakAffinity(), destination);
            }
        }
    }

    static void addAllowListedDestination(URI destination) {
        if (destination != null) {
            if (Logs.INVOCATION.isDebugEnabled()) {
                Logs.INVOCATION.debugf("DiscoveryEJBClientInterceptor: Allowlisting destination %s", destination);
            }
            allowlist.put(destination, System.nanoTime());
        }
    }

    static boolean isAllowListed(AbstractInvocationContext context, URI destination) {
        final Set<URI> invocationAllowlist = context.getAttachment(BL_KEY);
        if (invocationAllowlist != null && invocationAllowlist.contains(destination)) {
            return true;
        }

        // check if it is in the static allowlist
        if (!allowlist.containsKey(destination)) {
            return false;
        }

        // getAllowlist() will remove any allowlisted entries that are now > than ALLOWLIST_TIMEOUT
        // to check if any allowlisted destinations are now available to be used
        return getAllowlist().contains(destination);
    }

    static Set<URI> getAllowlist(){
        allowlist.entrySet().removeIf(e ->
        {
            return (System.nanoTime() - e.getValue()) > ALLOWLIST_TIMEOUT;
        });
    ...
    private List<Throwable> executeDiscovery(AbstractInvocationContext context) {
        ...
        if (affinity instanceof URIAffinity || affinity == Affinity.LOCAL) {
            if (! isAllowListed(context, affinity.getUri())) {
            ...
            if (! isAllowListed(context, weakAffinity.getUri())) {
    ...
    private List<Throwable> doFirstMatchDiscovery(AbstractInvocationContext context, final FilterSpec filterSpec, final FilterSpec fallbackFilterSpec) {
        ...
        final Set<URI> allowlist = getAllowlist();
